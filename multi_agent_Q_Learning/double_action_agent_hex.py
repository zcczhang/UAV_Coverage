from multi_agent_Q_Learning.hexagon_env import HexagonEnv
...
"""
A class that contains the logic to have two agents run together but this time it controls the two agents at the same
time.
...
class DoubleHexagonAgent:
    ...
    def give_reward_combined(self):  # adding reward function in the environment
        reward = 0
        if tuple(self.env0.state) in self.visited_0:
            reward -= 1
        elif tuple(self.env0.state) in self.visited_1:
            reward -= 2
        else:
            reward += 3
        if tuple(self.env1.state) in self.visited_0:
            reward -= 1
        elif tuple(self.env1.state) in self.visited_1:
            reward -= 2
        else:
            reward += 3
        return reward
    ...
    def set_action(self, action):  # sets and learns (this should probably be updated)
        ...
        self.visited_0.add(tuple(curr_state_0))
        self.combined_visited.add(tuple(curr_state_1))
        self.visited_1.add(tuple(curr_state_1))
